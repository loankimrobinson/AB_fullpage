...
output$week_text1 <- output$week_text <- renderText({
  req(input$week)
...
lapply(
 ...
 output$week_out <- renderText({
  HTML(paste0(input$week))
})
...
output$analytics_out <- renderUI({
  ...
  div(
    ...
    wellPanel(
      style = 'background-color:rgba(250,250,250,0.4);text-align:left;color:white;padding-top:0px;padding-bottom:0px;',
    fluidRow(
      column(width = 6,h1(htmlOutput("week_text1"))),
      #column(width = 2,h2(home_team_id)),
      column(width = 3, tags$button(
     ...
     # column(width = 2,h2( away_team_id ))
    )
    )
  )
))
